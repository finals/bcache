			       ...
			       const char *name)
{
	...
	for_each_cache(ca, d->c, i)
		bd_link_disk_holder(ca->bdev, d->disk); /* create symlinks between holding disk and slave bdev */
				 ...
				 unsigned id)
{
	...
	closure_get(&c->caching);  //cache_set中的引用计数caching加1
}
			      ...
			      sector_t sectors)
{
	...
	d->disk->fops		= &bcache_ops; //bcache设备的block_device_operations
	d->disk->private_data	= d;

	q = blk_alloc_queue(GFP_KERNEL); //分配bcache设备的request_queue
	if (!q)
		return -ENOMEM;
...
static void calc_cached_dev_sectors(struct cache_set *c)
{
	...
	list_for_each_entry(dc, &c->cached_devs, list) //遍历cache_set的后端设备，计算所有设备的扇区总数
...
int bch_cached_dev_attach(struct cached_dev *dc, struct cache_set *c, uint8_t *set_uuid)
{
	...
	if (dc->disk.c) {
		pr_err("Can't attach %s: already attached", dc->backing_dev_name);
		return -EINVAL;
	}

	if (test_bit(CACHE_SET_STOPPING, &c->flags)) {
		pr_err("Can't attach %s: shutting down", dc->backing_dev_name);
		return -EINVAL;
	}
	...
	if (!u) {
		if (BDEV_STATE(&dc->sb) == BDEV_STATE_DIRTY) {
			pr_err("Couldn't find uuid for %s in set", dc->backing_dev_name);
			return -ENOENT;
		}

		u = uuid_find_empty(c);
		if (!u) {
			pr_err("Not caching %s, no room for UUID", dc->backing_dev_name);
			return -EINVAL;
		}
	}
	...
	pr_info("Caching %s as %s on set %pU",
		dc->backing_dev_name, dc->disk.disk->disk_name,
				 ...
				 struct cached_dev *dc)
{
	...
	if (cached_dev_init(dc, sb->block_size << 9))  //初始化后端设备(struct cached_dev)
		goto err;
...
bool bch_cached_dev_error(struct cached_dev *dc)
{
	...
	pr_err("bcache: stop %s: too many IO errors on backing device %s\n",
		dc->disk.name, dc->backing_dev_name);
...
struct cache_set *bch_cache_set_alloc(struct cache_sb *sb)
{
	...
	if (!(c->devices = kzalloc(c->nr_uuids * sizeof(void *), GFP_KERNEL)) ||
	    ...
	    bch_bset_sort_state_init(&c->sort, ilog2(c->btree_pages)))  //用于bset中的sort操作
		goto err;
...
static const char *register_cache_set(struct cache *ca)
{
	...
	list_for_each_entry(c, &bch_cache_sets, list)
		if (!memcmp(c->sb.set_uuid, ca->sb.set_uuid, 16)) { //检查是否有重复的set_uuid
			       ...
			       const char *buffer, size_t size)
{
	...
	if (SB_IS_BDEV(sb)) {  //根据块设备超级块内容，判断是否为后端设备
		...
		mutex_lock(&bch_register_lock);
		register_bdev(sb, sb_page, bdev, dc); //注册后端设备
		mutex_unlock(&bch_register_lock);
	} else {   //缓存设备
