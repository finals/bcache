...
rw_attribute(writeback_idle_duration_msecs);
...
#define var(stat)		(dc->stat)
    ...
    if (attr == &sysfs_stop_when_cache_set_failed)
        return bch_snprint_string_list(buf, PAGE_SIZE,
                   bch_stop_on_failure_modes + 1, dc->stop_when_cache_set_failed);
...
#define d_strtoi_h(var)		sysfs_hatoi(var, dc->var)
	...
	if (attr == &sysfs_writeback_unconditional) {
        int v = strtoul_or_return(buf);

        if (v == 1) {
            dc->writeback_unconditional = true;
        } 
        if (v == 0) {
...
SHOW(__bch_cache_set)
{
    ...
    sysfs_printf(max_writeback_rate_when_idle, "%i", c->max_writeback_rate_when_idle);
...
STORE(__bch_cache_set)
{
	...
	sysfs_strtoul(copy_gc_enabled,		c->copy_gc_enabled);
        sysfs_strtoul(max_writeback_rate_when_idle, c->max_writeback_rate_when_idle);
	return size;
}
