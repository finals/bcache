...
uint64_t bch_next_delay(struct bch_ratelimit *d, uint64_t done)
{
	 ...
	 */
	if (time_before64(now + NSEC_PER_SEC * 5 / 2, d->next)) //now+2.5s 大于 d->next
		d->next = now + NSEC_PER_SEC * 5 / 2;               //下一次触发回写的时间最迟是now+2.5秒

	if (time_after64(now - NSEC_PER_SEC * 2, d->next)) //now-2s 小于 d->next
		d->next = now - NSEC_PER_SEC * 2;              //下一次触发回写的时间最迟是now-2秒

	return time_after64(d->next, now)  //下一次触发回写的延迟时间时间
