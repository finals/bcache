...
struct bucket {
	atomic_t	pin;
	uint16_t	prio;  //优先级，每次缓存命中会增加，所有bucket的优先级会周期性减少，用于LRU算法
	uint8_t		gen;
...
#define RECENT_IO	(1 << RECENT_IO_BITS)
    ...
    uint64_t        writeback_idle_duration_msecs;

	unsigned		writeback_rate_update_seconds;
	unsigned		writeback_rate_d_term;
	unsigned		writeback_rate_i_term_inverse; //表示1秒内出错的概率阈值，默认10000，表示1秒内高于1/10000的错误会记录下来
	unsigned		writeback_rate_p_term_inverse; //用于计算flush速率，默认40，表示速率为在40秒内能刷脏完成
...
{
    pr_info("cached_dev_put entry, dc->count: %d", (int)&dc->count);
	if (refcount_dec_and_test(&dc->count)) {
        pr_info("cached_dev_put schedule_work, dc->count: %d", (int)&dc->count);
